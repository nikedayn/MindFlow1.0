...
const LibraryTab = ({ routeKey, onDataChange, refreshCounter }) => {
  ...
  const renderEmptyComponent = () => {
    ...
    let subtitle = "Ви ще нічого не додали до цього розділу.";

    if (routeKey === 'tasks') {
      icon = "checkbox-outline";
      title = "Немає справ";
      subtitle = "Ваші важливі завдання з'являться тут.";
    } else if (routeKey === 'ideas') {
      icon = "bulb-outline";
      title = "Жодної ідеї";
      subtitle = "Зберігайте сюди все, що надихає.";
    } else if (routeKey === 'archive') {
      icon = "archive-outline";
      title = "Архів порожній";
      subtitle = "Тут будуть ваші завершені та заархівовані думки.";
    }

    return (
      <View style={styles.emptyContainer}>
        <Ionicons name={icon} size={80} color="#CAC4D0" />
  ...
  return (
    <View style={styles.sceneContainer}>
      <FlatList 
        ...
        renderItem={({ item }) => (
          <LibraryItem 
            ...
            onToggleComplete={async (i) => { await DataService.updateItem(i.id, { isCompleted: !i.isCompleted }); onDataChange(); }} 
          />
        )} 
                  ...
                  <View style={{marginTop: 16}}>
                    ...
                    {hasDueDate && (
                      <TouchableOpacity onPress={() => setShowDatePicker(true)} style={{marginTop: 8}}>
                        <Text style={{color: '#6750A4', textAlign: 'center'}}>Змінити дату</Text>
                      </TouchableOpacity>
                    )}
                  </View>
                )}
      ...
      {/* Контекстне меню дій (Bottom Sheet) */}
      <Modal 
        animationType="slide" 
        transparent 
            ...
            <TouchableWithoutFeedback>
              <View style={[styles.modalView, styles.bottomSheet]}>
                ...
                <TouchableOpacity style={styles.menuItem} onPress={() => handleQuickAction('delete')}>
                  ...
                  <Text style={[styles.menuText, {color: '#B3261E'}]}>Видалити</Text>
                </TouchableOpacity>
            ...
            <TouchableWithoutFeedback>
              <View style={styles.modalView}>
                ...
                <View style={styles.modalButtons}>
                  ...
                  <TouchableOpacity 
                    style={[styles.modalBtn, {backgroundColor: '#B3261E'}]} 
                    onPress={async () => { await DataService.deleteItem(selectedItem.id); setDeleteModalVisible(false); onDataChange(); }}
                  >
  ...
  return (
    <TabView
      ...
      renderScene={({ route }) => (
        <LibraryTab 
          routeKey={route.key} 
          onDataChange={() => setRefreshCounter(p => p+1)} 
      ...
      renderTabBar={props => (
        <TabBar 
          {...props} 
          indicatorStyle={{ backgroundColor: '#6750A4', height: 3 }} 
